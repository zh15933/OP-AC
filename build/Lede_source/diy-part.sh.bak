...
uci set network.lan.gateway='192.168.5.3'                    # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
uci set network.lan.broadcast='192.168.5.255'                # 旁路由设置 IPv4 广播（去掉uci前面的#生效）
...
sed -i "s/OpenWrt /Reimu_LM compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "${ZZZ_PATH}"
